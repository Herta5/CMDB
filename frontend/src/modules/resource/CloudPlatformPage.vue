...
onMounted(() => { polling = setInterval(() => { if (projects.currentProjectId && store.jobs.some(job => job.status === 'queued' || job.status === 'running')) void store.load(projects.currentProjectId, props.provider) }, 3000) })
onUnmounted(() => { if (polling) clearInterval(polling) })
</script>

<template>
  <section class="cloud-page">
    <header class="page-heading"><div><p class="page-eyebrow">云平台 / {{ platform.name }}</p><h1>{{ platform.name }} 资源</h1><p class="page-description">统一管理接入源、云资源地址、同步任务与失联生命周期。</p></div><button v-if="projects.currentProjectId" class="console-button is-primary" @click="createNew">添加接入源</button></header>
    ...
    <template v-else>
      ...
      <p v-if="store.mutationError" class="cloud-alert">{{ store.mutationError }}</p><p v-if="store.connectionMessage" class="cloud-success">{{ store.connectionMessage }}</p>
