...
const props = withDefaults(defineProps<{ provider: Provider; syncOnly?: boolean }>(), { syncOnly: false })
...
<template>
  <section class="cloud-page">
    ...
    <template v-else>
      ...
      <div v-else class="sync-toolbar"><span class="muted">{{ platform.name }} 接入源共 {{ store.sources.length }} 个</span><button class="console-button is-primary" @click="createNew">创建云同步</button></div>
