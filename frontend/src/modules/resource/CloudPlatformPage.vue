...
const platform = computed(() => ({ aliyun: { name: '阿里云', types: ['ecs', 'rds', 'slb', 'alb', 'nlb', 'gwlb'] }, aws: { name: 'AWS', types: ['ec2', 'rds', 'clb', 'alb', 'nlb', 'gwlb'] } }[selectedProvider.value]))
...
watch(() => [projects.currentProjectId, props.provider], ([projectId]) => { closeSourceDialog(); if (projectId) { if (props.syncOnly) void store.loadSyncManagement(Number(projectId)); else void store.load(Number(projectId), props.provider) } }, { immediate: true })
...
onUnmounted(() => { if (polling) clearInterval(polling); closeSourceDialog() })
</script>
