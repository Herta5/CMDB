...
const toResource = (v: ResourceDTO): CloudResource => ({ id: v.id, sourceId: v.source_id, provider: v.provider, resourceType: v.resource_type, externalId: v.external_id, name: v.name, region: v.region, zone: v.zone, cloudStatus: v.cloud_status, assetStatus: v.asset_status, instanceType: v.instance_type, vcpu: v.vcpu, memory: v.memory, storageType: v.storage_type, storageSizeGiB: v.storage_size_gib, lastSeenAt: v.last_seen_at, missingSince: v.missing_since, endpoints: (v.endpoints ?? []).map((e: Record<string, any>) => ({ id: e.id, kind: e.kind, address: e.address, port: e.port, protocol: e.protocol, resolvedIps: e.resolved_ips ?? [] })), disks: (v.disks ?? []).map((disk: Record<string, any>) => ({ id: disk.id, kind: disk.kind, type: disk.type, sizeGiB: disk.size_gib, device: disk.device, encrypted: disk.encrypted })) })
