...
async function loadAssets() {
  ...
  try {
    const params = {
      ...
      sort_by: sortBy.value, sort_order: sortOrder.value, page: page.value, page_size: pageSize.value,
    }
...
function applyFilters() { page.value = 1; void loadAssets() }
function clearFilters() { provider.value = ''; sourceID.value = ''; resourceType.value = ''; region.value = ''; cloudStatus.value = ''; assetStatus.value = ''; page.value = 1; void loadAssets() }
...
<template>
  <section>
    ...
    <template v-else-if="props.category === 'server'">
      <section class="server-search-panel">
        ...
        <div v-if="filtersOpen" class="server-filter-grid">
          <label>云平台<select v-model="provider"><option value="">全部</option><option value="aliyun">阿里云</option><option value="aws">AWS</option></select></label>
          <label>接入源<select v-model="sourceID"><option value="">全部</option><option v-for="option in sourceOptions" :key="option[0]" :value="option[0]">{{ option[1] }}</option></select></label>
          <label>类型<select v-model="resourceType"><option value="">ECS + EC2</option><option value="ecs">ECS</option><option value="ec2">EC2</option></select></label>
          <label>地域<input v-model="region" placeholder="输入地域，如 cn-shanghai"></label>
          <label>云端状态<select v-model="cloudStatus"><option value="">全部</option><option value="running">运行中</option><option value="stopped">已停止</option></select></label>
          <label>资产状态<select v-model="assetStatus"><option value="">全部</option><option value="active">正常</option><option value="lost">已失联</option></select></label>
      ...
      <section class="console-panel server-list-panel">
        <div class="panel-heading"><div><h2>服务器列表</h2><p class="muted">按资源名称升序</p></div><span class="muted">共 {{ total }} 项</span></div>
    ...
    <section v-else class="console-panel">
      <div class="panel-heading resource-toolbar"><h2>{{ category.title }}列表</h2><div><select v-model="assetStatus" aria-label="资产状态" @change="applyFilters"><option value="">全部状态</option><option value="active">正常</option><option value="lost">已失联</option></select><span class="muted">共 {{ total }} 项</span></div></div>
