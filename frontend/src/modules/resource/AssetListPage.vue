...
// 本页面提供项目边界内的只读资源列表；服务器与数据库共享高密度列表、服务端分页和只读详情抽屉。
...
type ListColumn = 'asset' | 'project' | 'source' | 'type' | 'instance_type' | 'vcpu' | 'memory' | 'disk_size' | 'engine' | 'engine_version' | 'storage_size' | 'region' | 'private_ip' | 'public_ip' | 'endpoint' | 'cloud_status' | 'asset_status' | 'last_seen_at'
...
const usesDenseList = computed(() => props.category === 'server' || props.category === 'database')
...
const defaultColumns = computed(() => props.category === 'database' ? databaseDefaultColumns : serverDefaultColumns)
const optionalColumns = computed(() => props.category === 'database' ? databaseOptionalColumns : serverOptionalColumns)
const listColumns = ref<ListColumn[]>([...serverDefaultColumns])
const hiddenColumns = ref<ListColumn[]>([])
const columnLabels: Record<ListColumn, string> = { asset: '资产', project: '项目', source: '来源', type: '类型', instance_type: '实例类型', vcpu: 'vCPU', memory: '内存', disk_size: '磁盘', engine: '数据库引擎', engine_version: '引擎版本', storage_size: '存储容量', region: '地域', private_ip: '内网 IP', public_ip: '公网 IP', endpoint: '访问地址', cloud_status: '云端状态', asset_status: '资产状态', last_seen_at: '最近发现时间' }
...
function loadColumnSettings() {
  ...
  try {
    ...
    if (saved.order?.length === defaultColumns.value.length && saved.order.every(column => defaultColumns.value.includes(column))) listColumns.value = saved.order
...
async function loadAssets() {
  ...
  try {
    const params = {
      ...
      keyword: keyword.value, engine: props.category === 'database' ? engine.value : '', region: region.value, cloud_status: cloudStatus.value, asset_status: assetStatus.value,
      // 资源列表不提供排序切换，所有请求固定按资产名称升序，避免分页刷新时顺序变化。
      sort_by: 'name', sort_order: 'asc', page: usesDenseList.value ? page.value : 1, page_size: usesDenseList.value ? pageSize.value : 200,
    }
    // 负载均衡仍维持既有“每项目、每类型最多 200 条”读取；服务器和数据库统一由服务端分页。
    if (!usesDenseList.value) {
...
async function loadFilterOptions() {
  ...
  if (!usesDenseList.value || !targets.length) return
  try {
...
function applyFilters() {
  provider.value = draftProvider.value; sourceID.value = draftSourceID.value; resourceType.value = props.category === 'server' ? draftResourceType.value : ''; engine.value = props.category === 'database' ? draftEngine.value.trim() : ''
...
function clearFilters() {
  provider.value = ''; sourceID.value = ''; resourceType.value = ''; engine.value = ''; region.value = ''; cloudStatus.value = ''; assetStatus.value = ''
  draftProvider.value = ''; draftSourceID.value = ''; draftResourceType.value = ''; draftEngine.value = ''; draftRegion.value = ''; draftCloudStatus.value = ''; draftAssetStatus.value = ''
  page.value = 1
  void loadAssets()
}
function removeFilter(key: string) {
  ...
  draftProvider.value = provider.value; draftSourceID.value = sourceID.value; draftResourceType.value = resourceType.value; draftEngine.value = engine.value
...
watch(() => [projects.currentProjectId, props.category], () => {
  ...
  keywordInput.value = ''; keyword.value = ''; provider.value = ''; sourceID.value = ''; resourceType.value = ''; engine.value = ''; region.value = ''; cloudStatus.value = ''; assetStatus.value = ''
  draftProvider.value = ''; draftSourceID.value = ''; draftResourceType.value = ''; draftEngine.value = ''; draftRegion.value = ''; draftCloudStatus.value = ''; draftAssetStatus.value = ''
  page.value = 1
  loadColumnSettings()
  void loadAssets()
...
const activeFilterCount = computed(() => [provider.value, sourceID.value, resourceType.value, engine.value, region.value, cloudStatus.value, assetStatus.value].filter(Boolean).length)
...
<template>
  <section>
    <header class="page-heading"><div><p class="page-eyebrow">资产列表</p><h1>{{ category.title }}</h1></div><button v-if="props.category === 'load_balancer'" class="console-button" :disabled="!hasAssetScope || loading" @click="loadAssets">刷新列表</button></header>
    <div v-if="!hasAssetScope" class="console-panel page-state"><span class="state-symbol">▦</span><h3>请先选择项目</h3><p>资产必须在明确的项目边界内查看。</p></div>
    <template v-else-if="usesDenseList">
      <section class="console-panel server-list-panel">
        ...
        <div class="server-search-panel">
          <form class="server-search" @submit.prevent="search"><input v-model="keywordInput" :aria-label="`搜索${category.title}`" :placeholder="props.category === 'server' ? '搜索资源名称、实例 ID、实例类型或 IP 地址' : '搜索资源名称、实例 ID、实例类型或访问地址'"><button class="console-button is-primary" type="submit">搜索</button><button class="console-button" type="button" :aria-expanded="filtersOpen" @click="filtersOpen = !filtersOpen">筛选<span v-if="activeFilterCount"> · {{ activeFilterCount }}</span></button><button class="console-button" type="button" :aria-expanded="columnsOpen" @click="columnsOpen = !columnsOpen">列设置</button><button class="console-button" type="button" :disabled="loading" @click="loadAssets">刷新列表</button></form>
          ...
          <div v-if="filtersOpen" class="server-filter-grid">
            ...
            <label v-else>数据库引擎<input v-model="draftEngine" aria-label="数据库引擎" placeholder="输入云端引擎值，如 PostgreSQL"></label>
    ...
    <section v-else class="console-panel">
      ...
      <div v-else class="table-scroll"><table class="console-table cloud-table"><thead><tr><th>资产</th><th v-if="viewingAllProjects">项目</th><th>云平台</th><th>类型</th><th>区域 / 可用区</th><th>云端状态</th><th>资产状态</th><th>访问地址</th></tr></thead><tbody><tr v-for="item in resources" :key="`${item.resourceType}-${item.id}`"><td><strong>{{ item.name || item.externalId }}</strong><small>{{ item.externalId }}</small></td><td v-if="viewingAllProjects">{{ item.projectName }}</td><td>{{ providerLabel(item.provider) }}</td><td>{{ resourceTypeLabel(item.resourceType) }}</td><td>{{ display(item.region) }} / {{ display(item.zone) }}</td><td>{{ display(item.cloudStatus) }}</td><td><span class="status-badge" :class="{ 'is-lost': item.assetStatus === 'lost' }">{{ item.assetStatus === 'lost' ? '已失联' : '正常' }}</span></td><td><div v-for="endpoint in item.endpoints" :key="`${endpoint.kind}-${endpoint.address}-${endpoint.port}`" class="endpoint"><span>{{ endpoint.kind }}</span>{{ endpoint.address }}<b v-if="endpoint.port">:{{ endpoint.port }}</b><small v-if="endpoint.resolvedIps.length">解析：{{ endpoint.resolvedIps.join(', ') }}</small></div><span v-if="!item.endpoints.length">—</span></td></tr></tbody></table></div>
    </section>
    ...
    <div v-if="selected" class="resource-drawer-backdrop" @click.self="closeDetails"><aside class="resource-drawer" role="dialog" aria-modal="true" :aria-label="`${category.title}详情`"><header><div><p class="page-eyebrow">{{ category.title }}详情</p><h2>{{ selected.name || selected.externalId }}</h2></div><button class="dialog-close" aria-label="关闭详情" @click="closeDetails">×</button></header>
      ...
      <section><h3>实例配置</h3><dl><div><dt>实例类型</dt><dd>{{ specificationValue(selected.instanceType) }}</dd></div><div><dt>vCPU</dt><dd>{{ valueWithUnit(selected.vcpu, 'vCPU') }}</dd></div><div><dt>内存</dt><dd>{{ memoryWithUnit(selected.memory) }}</dd></div></dl></section>
