...
const memoryGiB = (memory?: number | null) => {
...
<template>
  <section>
    ...
    <section v-else class="console-panel">
      ...
      <div v-else class="table-scroll"><table class="console-table cloud-table"><thead><tr><th>资产</th><th v-if="viewingAllProjects">项目</th><th>云平台</th><th>类型</th><th v-if="props.category === 'server' || props.category === 'database'">实例规格</th><th v-if="props.category === 'server'">磁盘</th><th v-if="props.category === 'database'">存储</th><th>区域 / 可用区</th><th>云端状态</th><th>资产状态</th><th>访问地址</th></tr></thead><tbody><tr v-for="item in resources" :key="`${item.resourceType}-${item.id}`"><td><strong>{{ item.name || item.externalId }}</strong><small>{{ item.externalId }}</small></td><td v-if="viewingAllProjects">{{ item.projectName }}</td><td>{{ item.provider === 'aliyun' ? '阿里云' : 'AWS' }}</td><td>{{ item.resourceType.toUpperCase() }}</td><td v-if="props.category === 'server' || props.category === 'database'">{{ specificationValue(item.instanceType) }} · {{ specificationValue(item.vcpu) }} vCPU · {{ memoryGiB(item.memory) }} GiB</td><td v-if="props.category === 'server'">{{ diskSummary(item) }}</td><td v-if="props.category === 'database'">{{ storageSummary(item) }}</td><td>{{ display(item.region) }} / {{ display(item.zone) }}</td><td>{{ display(item.cloudStatus) }}</td><td><span class="status-badge" :class="{ 'is-lost': item.assetStatus === 'lost' }">{{ item.assetStatus === 'lost' ? '已失联' : '正常' }}</span></td><td><div v-for="endpoint in item.endpoints" :key="`${endpoint.kind}-${endpoint.address}-${endpoint.port}`" class="endpoint"><span>{{ endpoint.kind }}</span>{{ endpoint.address }}<b v-if="endpoint.port">:{{ endpoint.port }}</b><small v-if="endpoint.resolvedIps.length">解析：{{ endpoint.resolvedIps.join(', ') }}</small></div><span v-if="!item.endpoints.length">—</span></td></tr></tbody></table></div>
    </section>
  </section>
</template>
