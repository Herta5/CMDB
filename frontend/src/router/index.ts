...
const routes: RouteRecordRaw[] = [
  {
    ...
    children: [
      ...
      { path: 'projects', name: 'ProjectList', component: () => import('@/modules/project/ProjectListPage.vue'), meta: { title: '项目管理', requiresProjectAdmin: true } },
...
router.beforeEach(async (to) => {
  ...
  // 全局管理页面在路由边界拒绝普通用户，不仅依赖侧栏隐藏。
  if (to.meta.requiresSystemAdmin && auth.currentUser?.globalRole !== 'system_admin') return '/assets/servers'

  // 项目管理入口以当前项目角色为准；刷新或直达时先恢复服务端授权列表。
  if (to.meta.requiresProjectAdmin && auth.currentUser?.globalRole !== 'system_admin') {
    const projects = useProjectStore()
    if (projects.listState !== 'ready') await projects.loadProjects()
    if (projects.currentProject?.currentRole !== 'project_admin') return '/assets/servers'
  }
