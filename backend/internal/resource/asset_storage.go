...
func resourceFromRow(row assetRow, table string) Resource {
	...
	value := Resource{AssetBase: row.AssetBase, Engine: row.Engine, EngineVersion: row.EngineVersion, NetworkType: row.NetworkType, InstanceType: row.InstanceType, StorageType: row.StorageType, Endpoints: endpoints, Disks: disks}
	if row.VCPU.Valid {
